...
const Actors = (function () {
  ...
  /* Clasifica cada estado de ánimo narrativo en una de las 4 familias
     de aura definidas en CSS, para que el pulso de luz del token
     comunique correctamente el estado emocional del cliente. */
  const MOOD_FAMILY = {
    'Estresado':'agitado','Hiperalerta':'agitado','Tenso':'agitado','Dolorido':'agitado',
    'Molesta':'agitado','Rígido':'agitado','Determinado':'agitado','Apresurada':'agitado',
    'Ansiosa':'ansioso','Ansioso':'ansioso','Abrumada':'ansioso','Nerviosa':'ansioso',
    'Disperso':'ansioso','Inquieto':'ansioso','Inquieta':'ansioso','Preocupado':'ansioso',
    'Preocupada':'ansioso','Vulnerable':'ansioso','Sensible':'ansioso','Exigente':'ansioso',
    'Irónicamente Estresada':'ansioso','Irónicamente Tensa':'ansioso','Desesperada':'ansioso',
    'Agotado':'cansado','Fatigada':'cansado','Fatigado':'cansado','Cansado':'cansado',
    'Desconectada':'cansado','Saturada':'cansado','Sobrecargado':'cansado','Aletargado':'cansado',
    'Desgastada':'cansado','Desvelada':'cansado','Quemado':'cansado','Melancólica':'cansado',
    'Agotado Físicamente':'cansado','Burnout severo':'cansado','Empática pero Agotada':'cansado',
    'Fatiga de gira':'cansado'
  };
  function auraFamilyFor(mood) {
    return MOOD_FAMILY[mood] || 'neutral';
  }
  ...
  function create(cfg) {
    ...
    const family = cfg.role === 'staff' ? null : auraFamilyFor(cfg.mood);
    el.className = 'actor role-' + (cfg.role === 'staff' ? 'staff' : 'customer') + (family ? ' aura-' + family : '');
    el.setAttribute('role', 'img');
    el.setAttribute('aria-label', (cfg.role === 'staff' ? 'Personal' : 'Cliente') + ' ' + cfg.name);

    // Aura: aro de luz pulsante detrás del token, cuyo color/cadencia
    // vienen definidos por la familia de mood vía clase CSS (aura-*).
    const aura = document.createElement('div');
    aura.className = 'actor-aura';

    const avatar = document.createElement('div');
    avatar.className = 'actor-avatar';
    const color = cfg.role === 'staff' ? null : V.avatarColorFor(cfg.name);
    if (color) {
      avatar.style.setProperty('--token-c1', color);
      avatar.style.setProperty('--token-c2', shade(color, -22));
    }
  ...
  /* Transiciona el aura de un cliente al tono "calma" (verde salvia,
     respiración lenta) tras completar con éxito su tratamiento. */
  function setMoodSuccess(id) {
    const a = registry[id];
    if (!a) return;
    a.el.className = a.el.className.replace(/\baura-\w+\b/g, '').trim();
    a.el.classList.add('mood-success');
  }
  ...
  return { create, remove, get, moveTo, setState, waitForArrival, setMoodSuccess };
})();
V.Actors = Actors;
...
/* ---- Mobiliario ambiental de la escena (velas, letrero, TV, difusor, etc.) ----
   El orden importa: piezas más al fondo (letrero, TV, ventanales) primero,
   piezas de primer plano (velas, plantas, cortinas) después, así el
   z-index natural del DOM las apila correctamente dentro de #decor-layer. */
function buildRainDrops(count, prefix) {
  let html = '';
  for (let i = 0; i < count; i++) {
    const left = V.randInt(0, 100);
    const dur = (V.randInt(55, 110) / 100).toFixed(2);
    const delay = (Math.random() * 2.4).toFixed(2);
    html += `<div class="${prefix}-drop" style="left:${left}%; animation-duration:${dur}s; animation-delay:${delay}s;"></div>`;
  }
  return html;
}
...
function spawnDecor() {
  ...
  const pieces = [
    /* Ventanal panorámico de fondo: skyline nocturno + lluvia + relámpagos.
       Va primero para quedar detrás de todo el resto del mobiliario. */
    { cls: 'window-wall', style: '', html:
        '<div class="ww-sky"></div><div class="ww-stars"></div>' +
        '<div class="ww-skyline-far"></div><div class="ww-skyline-near"></div>' +
        '<div class="ww-lights"></div><div class="ww-haze"></div>' +
        '<div class="ww-rain">' + buildRainDrops(46, 'ww') + '</div>' +
        '<div class="ww-glass-streaks"></div>' +
        '<div class="ww-frame"></div><div class="ww-frame-h"></div>' +
        '<div class="ww-flash" id="ww-flash"></div>' },

    /* Letrero "VALTARA" grabado en madera, retroiluminado */
    { cls: 'decor-sign', style: '', html:
        '<div class="sign-panel"><div class="sign-glow"></div><span class="sign-text">VALTARA</span></div>' +
        '<div class="sign-underline"></div>' },

    /* Pantalla ambiental con "video" de olas simulado */
    { cls: 'decor-tv', style: '', html:
        '<div class="tv-frame"><div class="tv-screen"><div class="tv-waves"></div></div></div><div class="tv-glow"></div>' },
    ...
    /* Difusor de aromaterapia sobre el mostrador, con vapor animado */
    { cls: 'decor-diffuser', style: 'left:50%; margin-left:34px;', html:
        '<div class="df-body"><div class="df-led"></div></div>' +
        '<div class="df-mist"><div class="df-puff"></div><div class="df-puff" style="animation-delay:1.1s; left:44%;"></div><div class="df-puff" style="animation-delay:2.2s; left:58%;"></div></div>' },
...
/* ---- Relámpagos ocasionales: destello sincronizado dentro del cristal
   y sobre la escena completa, con un eco tenue de segundo golpe a veces
   (como una tormenta real). No depende del audio; es puramente visual. ---- */
const Weather = (function () {
  let stormTimer = null;

  function strikeOnce() {
    if (V.prefersReducedMotion()) return;
    const glass = V.$('#ww-flash');
    const room = V.$('#storm-flash');
    [glass, room].forEach(el => {
      if (!el) return;
      el.classList.remove('strike');
      void el.offsetWidth;
      el.classList.add('strike');
    });
    // Eco de segundo relámpago más débil, ~35% de las veces
    if (Math.random() < 0.35) {
      setTimeout(() => {
        [glass, room].forEach(el => {
          if (!el) return;
  ...
  function scheduleNext() {
    ...
    // Relámpagos poco frecuentes: cada 20-55s, para que se sientan como
    // un evento especial de clima y no una distracción constante.
    const nextIn = V.randInt(20000, 55000);
  ...
  function startLightning() {
    clearTimeout(stormTimer);
    // Primer relámpago con retraso inicial para no saludar al jugador de golpe
    stormTimer = setTimeout(() => {
      strikeOnce();
      scheduleNext();
    }, V.randInt(9000, 18000));
  }
...
const Reception = (function () {
  ...
  async function bringNextCustomer() {
    ...
    Actors.create({ id: custId, name: profile.name, role: 'customer', x: entrySide, y: DESK_Y, mood: profile.mood });
  ...
  function maybeSpawnPeakHourWaiting() {
    ...
    Actors.create({ id: wId, name: profile.name, role: 'customer', x: 88, y: DESK_Y + 14, mood: profile.mood });
  ...
  function handleChoice(service) {
    ...
    if (correct) {
      ...
      if (currentCustomerId) {
        Actors.setState(currentCustomerId, 'talking');
        Actors.setMoodSuccess(currentCustomerId);
      }
    } else {
...
/* ================================================================
   ...
   ================================================================ */
/* Fase Beta: el catálogo completo de recompensas es un teaser. Ningún
   nivel es canjeable todavía — todos quedan marcados con `locked: true`
   y muestran un mensaje "Próximamente" en vez del botón de WhatsApp,
   sin importar cuántas monedas o logros acumule el jugador. */
const REWARD_TIERS = [
  { threshold: 100000, icon: '🌸', name: 'Esencia a tu elección', desc: 'Canjea 100,000 ◈ por la esencia aromática que prefieras.', locked: true },
  { threshold: 500000, icon: '🕯️', name: 'Vela aromática a tu elección', desc: 'Canjea 500,000 ◈ por una vela aromática de tu elección.', locked: true },
  { threshold: 1000000, icon: '🎁', name: 'Regalo sorpresa', desc: 'Al llegar a 1,000,000 ◈ te tenemos preparado un regalo sorpresa que el equipo asignará directamente.', showClaim: true, locked: true },
  { threshold: 1500000, icon: '🏆', name: 'Diploma Valtara + sesión de esferas chinas con aromaterapia', desc: 'Completa todos los logros y alcanza 1,500,000 ◈ para obtener tu diploma Valtara y una sesión gratuita de esferas chinas con aromaterapia.', requiresAllAchievements: true, showClaim: true, locked: true }
];
...
const Views = (function () {
  ...
  /* Catálogo de recompensas — Fase Beta.
     Ningún nivel es canjeable todavía. Cada tarjeta sigue mostrando el
     progreso real del jugador (para que acumular monedas siga siendo
     motivador), pero el candado y el mensaje "Próximamente" reemplazan
     por completo cualquier botón de reclamo o contacto por WhatsApp. */
  function renderRewards() {
    ...
    const rows = REWARD_TIERS.map(tier => {
      ...
      const progressMet = reached && allAchvDone;
      const pct = V.clamp(Math.round((s.coins / tier.threshold) * 100), 0, 100);
      return `
        <div class="reward-tier reward-locked ${progressMet ? 'progress-met' : ''}">
          <div class="reward-tier-icon locked-icon">
            <span class="rt-lock-badge">🔒</span>${tier.icon}
          </div>
          <div class="reward-tier-info">
            ...
            <div class="rt-progress-track"><div class="rt-progress-fill" style="width:${pct}%;"></div></div>
            <div class="rt-progress-label">${progressMet ? 'Meta alcanzada · en espera de habilitación' : pct + '% del camino'}</div>
          </div>
        </div>
        <div class="reward-soon">
          <span class="rs-lock">🔒</span>
          <div>
            <div class="rs-title">PRÓXIMAMENTE</div>
            <p>Fase de prueba. Sigue acumulando monedas para cuando habilitemos estas recompensas.</p>
          </div>
        </div>
      `;
    }).join('');
    ...
    Modal.open(`
      <div class="modal-handle"></div>
      <div class="modal-title">✦ Catálogo de recompensas <span class="beta-pill">BETA</span></div>
      <div class="modal-sub">Valtara está en fase de prueba. Tus monedas ◈ se convertirán más adelante en Gevizz Coins — por ahora, ningún nivel es canjeable todavía. Sigue jugando: tu progreso ya está contando.</div>
